error id: file:///C:/Users/dell/OneDrive%20-%20Egypt%20University%20of%20Informatics/Desktop/CTRL%20ALT%20CHATBOT1/CTRL%20ALT%20CHATBOT/CTRL%20ALT%20CHATBOT/CTRL%20ALT%20CHATBOT/ScalaChatbotGUI/src/main/scala/ChatbotEngine.scala:`<none>`.
...
object ChatbotEngine {
  ...
  def evaluateQuizAnswer(userAnswerRaw: String, choices: List[String], correctAnswerText: String): String = {
    ...
    // Step 1: Determine what text the user's answer corresponds to from the choices
    val chosenAnswerTextOpt: Option[String] = 
      if (userAnswerProcessed.length == 1 && "abcd".take(choices.length).contains(userAnswerProcessed)) {
        ...
        // This 'if' was to prevent IndexOutOfBounds if choices.length < choiceIndex+1,
        // but the outer if already ensures choiceIndex is valid for "abcd".take(choices.length)
        Some(lowerChoices(choiceIndex)) 
      } else if (userAnswerProcessed.length == 1) { // Single letter, but not a valid choice letter
        None // Will lead to INVALID_CHOICE_LETTER or CHOICE_NOT_FOUND based on context
    ...
    case Some((topic: String, sessionQuestions: List[QuizQuestion], answersSoFar: List[Boolean], currentQuestionIdx: Int, missedSoFar: List[QuizQuestion])) =>
      ...
      currentQuestionOpt match {
        case Some(currentQuestionTuple: QuizQuestion) =>
          ...
          } else { // Process as a potential answer OR a hint request
