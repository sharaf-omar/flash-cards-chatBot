file:///C:/Users/dell/OneDrive%20-%20Egypt%20University%20of%20Informatics/Desktop/CTRL%20ALT%20CHATBOT/ScalaChatbotGUI/src/main/scala/ChatbotEngine.scala
...
action parameters:
uri: file:///C:/Users/dell/OneDrive%20-%20Egypt%20University%20of%20Informatics/Desktop/CTRL%20ALT%20CHATBOT/ScalaChatbotGUI/src/main/scala/ChatbotEngine.scala
text:
```scala
import java.io._
...
object ChatbotEngine {
  ...
  def evaluateQuizAnswer(userAnswerRaw: String, choices: List[String], correctAnswerText: String): Option[Boolean] = {
    ...
    def promptForValidQuizAnswer(choices: List[String],  correctAnswerText: String): String = {
      @scala.annotation.tailrec
      def loop(): String = {
        print("> ")
        val userInput = Try(StdIn.readLine()).getOrElse("")
        evaluateQuizAnswer(userInput, choices, correctAnswerText) match {
          case Some(_) => userInput
          case None =>
            println("Invalid input. Please enter 'a', 'b', 'c', 'd' or the full text of your answer.")
            loop()
        }
      }
      loop()
  ...
  def handleUserInput(
    ...
    currentQuizSessOpt match {
      case Some((topic, sessionQuestions, answersSoFar, currentQuestionIdx, missedSoFar)) =>
        val currentQuestion = sessionQuestions(currentQuestionIdx)
        val (qText, qChoices, qCorrectAnswer, qNote, qExplanation) = currentQuestion
        println(presentQuizQuestion(currentQuestion, currentQuestionIdx + 1))
        val userInput = promptForValidQuizAnswer(qChoices, qCorrectAnswer)
      ...
      def loop(currentState: ChatbotState): Unit = {
        ...
        Try(StdIn.readLine()) match {
          case scala.util.Success(userInputStringOrNull) =>
            ...
            val (_, _, currentQuizSessOpt, _, _, _) = currentState

            // If in a quiz session, keep prompting until valid answer
            currentQuizSessOpt match {
              case Some((topic, sessionQuestions, answersSoFar, currentQuestionIdx, missedSoFar)) =>
                val currentQuestion = sessionQuestions(currentQuestionIdx)
                val (qText, qChoices, qCorrectAnswer, qNote, qExplanation) = currentQuestion
                println(presentQuizQuestion(currentQuestion, currentQuestionIdx + 1))
                val quizAnswer = promptForValidQuizAnswer(qChoices, qCorrectAnswer)
                val (response, nextState) = handleUserInput(quizAnswer, currentState, initialExplanations, initialQuizBank)
                println(response)
                if (!farewellKeywords.contains(quizAnswer.toLowerCase) && response != "Goodbye! Happy learning!" && !response.startsWith("Goodbye ")) {
                  loop(nextState)
                }
              case None =>
