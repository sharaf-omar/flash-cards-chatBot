error id: file:///C:/Users/dell/OneDrive%20-%20Egypt%20University%20of%20Informatics/Desktop/CTRL%20ALT%20CHATBOT1/CTRL%20ALT%20CHATBOT/CTRL%20ALT%20CHATBOT/CTRL%20ALT%20CHATBOT/ScalaChatbotGUI/src/main/scala/ChatbotEngine.scala:find.
file:///C:/Users/dell/OneDrive%20-%20Egypt%20University%20of%20Informatics/Desktop/CTRL%20ALT%20CHATBOT1/CTRL%20ALT%20CHATBOT/CTRL%20ALT%20CHATBOT/CTRL%20ALT%20CHATBOT/ScalaChatbotGUI/src/main/scala/ChatbotEngine.scala
empty definition using pc, found symbol in pc: find.
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -java/io/lowerChoices/find.
	 -java/io/lowerChoices/find#
	 -java/io/lowerChoices/find().
	 -com/github/tototoshi/csv/lowerChoices/find.
	 -com/github/tototoshi/csv/lowerChoices/find#
	 -com/github/tototoshi/csv/lowerChoices/find().
	 -lowerChoices/find.
	 -lowerChoices/find#
	 -lowerChoices/find().
	 -scala/Predef.lowerChoices.find.
	 -scala/Predef.lowerChoices.find#
	 -scala/Predef.lowerChoices.find().
offset: 26389
uri: file:///C:/Users/dell/OneDrive%20-%20Egypt%20University%20of%20Informatics/Desktop/CTRL%20ALT%20CHATBOT1/CTRL%20ALT%20CHATBOT/CTRL%20ALT%20CHATBOT/CTRL%20ALT%20CHATBOT/ScalaChatbotGUI/src/main/scala/ChatbotEngine.scala
text:
```scala
import java.io._
...
object ChatbotEngine {
  ...
  val conceptKeywords = List(
      "c","c++", "javascript", "java","algorithm", "scala", "class", "arrays", "variables", "dsa", "oop", "loops", "inheritance", "polymorphism",
      "closure", "list", "array", "interface", "function", "pointer", "dictionary", "abstraction", "virtual function", "module", "prototype",
      "structure", "lambda coo", "encapsulation", "template", "set", "linked list", "exception handling", "list comprehension", "overloading",
  ...
  def isWholeWordMatchManual(text: String, phrase: String): Boolean = {
    val textLower = text.toLowerCase
    val phraseLower = phrase.toLowerCase
    var found = false
    var index = textLower.indexOf(phraseLower)
    while (index != -1 && !found) {
      val charBeforeOK = if (index == 0) true else !textLower(index - 1).isLetterOrDigit
      val charAfterOK = if (index + phraseLower.length == textLower.length) true else !textLower(index + phraseLower.length).isLetterOrDigit
  ...
  val (foundConceptsFromPhrases, textAfterConceptExtraction) =
    sortedCanonicalConceptKeywords.foldLeft((Set.empty[String], normalizedInput)) {
      case ((accFoundConcepts, currentText), conceptKey) =>
        if (isWholeWordMatchManual(currentText, conceptKey)) {
          (accFoundConcepts + conceptKey, currentText.replace(conceptKey, " ")) 
        } else {
          (accFoundConcepts, currentText)
        }
    }

  val singleWordOrAliasTokens = textAfterConceptExtraction
    .split(" ")
    .filter(_.nonEmpty)
    .flatMap { token =>
      variationToCanonical.get(token) match {
        case Some(canonicalValueFromMap) =>

          if (conceptKeywords.contains(canonicalValueFromMap) || languageKeywords.contains(canonicalValueFromMap) || knownKeywords.contains(canonicalValueFromMap)) {
            Some(canonicalValueFromMap)
          } else {
            // This case means `token` was an alias for something (`canonicalValueFromMap`)
            // but that `canonicalValueFromMap` itself isn't a recognized concept/keyword.
            // This shouldn't happen if `variationToCanonical` is built correctly from `conceptKeywords`.
            // We can choose to ignore it or log a warning. For now, ignore.
            None 
          }
        case None =>
          // Token is not an alias in variationToCanonical.
          // Check if the token itself is a general known keyword (action word or a canonical concept itself).
          if (knownKeywords.contains(token)) { // knownKeywords includes conceptKeywords, languageKeywords etc.
            Some(token)
          } else {
            None
          }
  ...
  (foundConceptsFromPhrases ++ singleWordOrAliasTokens).toList.distinct
}
  ...
  def determineIntent(tokens: List[String], rawInput: String): IntentInfo = {
    ...
    val (baseIntent, baseConcepts, baseLangOpt) = {
        if (isExplain) ("explain", foundConcepts.distinct, mainTopic)
        else if (isQuiz) ("quiz", foundConcepts.distinct, mainTopic)
  ...
  // Return type will be String: "CORRECT", "INCORRECT_VALID_CHOICE", "INVALID_CHOICE_LETTER", "CHOICE_NOT_FOUND"
  def evaluateQuizAnswer(userAnswerRaw: String, choices: List[String], correctAnswerText: String): String = {
    val userAnswerProcessed = userAnswerRaw.trim.toLowerCase
    val lowerCorrectAnswerText = correctAnswerText.toLowerCase
    val lowerChoices = choices.map(_.toLowerCase)
    ...
    def isSimilar(s1: String, s2: String, threshold: Double = 0.2): Boolean = {
      if (s1.trim.isEmpty || s2.trim.isEmpty) return false // Prevent division by zero if maxLength is 0
    ...
    val chosenAnswerTextOpt: Option[String] = 
      if(userAnswerProcessed.length == 1 && userAnswerProcessed.isDigit){
        
      }

      else if (userAnswerProcessed.length == 1 && "abcd".take(choices.length).contains(userAnswerProcessed)) {
        val choiceIndex = "abcd".indexOf(userAnswerProcessed)
        Some(lowerChoices(choiceIndex)) 
      } else if (userAnswerProcessed.length == 1) { 
        None 
      }else if(userAnswerProcessed == "hint" ||userAnswerProcessed.contains(" hint") || userAnswerProcessed.contains(" help") || userAnswerProcessed.contains(" tip") || userAnswerProcessed.contains(" hint ") || userAnswerProcessed.contains(" help ") || userAnswerProcessed.contains(" tip ") || userAnswerProcessed.contains("hint ") || userAnswerProcessed.contains("help ") || userAnswerProcessed.contains("tip ")){
        return "HINT_REQUESTED" // User asked for a hint
      } 
      else { // User typed full text
        // Check if the full text is similar to any of the choices
        lowerChoices.@@find(choice => choice == userAnswerProcessed || isSimilar(userAnswerProcessed, choice))
      }

    // Step 2: Evaluate based on chosenAnswerTextOpt
    chosenAnswerTextOpt match {
      case Some(chosenText) => // User's input mapped to a valid choice text
        if (chosenText == lowerCorrectAnswerText || isSimilar(chosenText, lowerCorrectAnswerText)) {
          "CORRECT"
        } else {
          "INCORRECT_VALID_CHOICE"
        }
      case None => // User's input did not map to any valid choice
        if (userAnswerProcessed.length == 1 && !"abcd".take(choices.length).contains(userAnswerProcessed)) {
          // It was a single letter, but not 'a', 'b', 'c', or 'd' (or out of range for current choices)
          "INVALID_CHOICE_LETTER"
        } else {
          // It was full text that didn't match any choice, or an invalid letter not caught above
          "CHOICE_NOT_FOUND"
        }
    }
  }
  ...
  def analyzeQuizPerformance(log: InteractionLog): String = {
      ...
      val scores = quizEndResponses.flatMap { case (_, _, response) =>
        ...
        scorePattern.findFirstMatchIn(response.toLowerCase).map { m =>
  ...
  val (currentLog, currentSeqNum, currentQuizSessOpt, currentSessionPrefs, lastActionCtx, currentUserNameOpt) = chatbotState
  ...
  if (input.contains("remember my name") || input.contains("what is my name") || input.contains("who am i")||
    ...
    currentSessionPrefs.get("session_username") match {
      case Some(name) =>
        ...
        return (r, (l,s, currentQuizSessOpt, currentSessionPrefs, None, currentUserNameOpt))
      case None =>
  ...
  def startNewQuiz(topic: String, numQuestions: Int = 5): (String, ChatbotState) = {
    selectQuizQuestions(topic, numQuestions, quizBank) match {
      case Some(selectedQuestions) if selectedQuestions.nonEmpty =>
        ...
        (response, (updatedLog, nextSeq, Some(newQuizSession), currentSessionPrefs, Some("quiz_started" -> topic), currentUserNameOpt))
      case _ =>
        ...
        val (updatedLog, nextSeq) = logInteraction(rawInput, response, currentLog, currentSeqNum)
        (response, (updatedLog, nextSeq, None, currentSessionPrefs, lastActionCtx, currentUserNameOpt))
    }
  }
      ...
      currentQuestionOpt match {
        case Some(currentQuestionTuple: QuizQuestion) =>
          ...
          if(input.toLowerCase == "clear") {
              ...
              val (updatedLog, nextSeq) = logInteraction(rawInput, response, currentLog, currentSeqNum)
              (response, (updatedLog, nextSeq, None, currentSessionPrefs, Some("quiz_exited" -> topic), currentUserNameOpt))
          } else {
            evaluateQuizAnswer(input, qChoices, qCorrectAnswer) match {
              case "HINT_REQUESTED" => 
                ...
                val (updatedLog, nextSeq) = logInteraction(rawInput, response, currentLog, currentSeqNum)
                (response, (updatedLog, nextSeq, currentQuizSessOpt, currentSessionPrefs, lastActionCtx, currentUserNameOpt)) // Re-ask, don't advance
                
              case "CORRECT" =>
                val feedback = "Correct!"
                val updatedAnswers = answersSoFar :+ true
                val updatedMissed = missedSoFar // No change if correct
                val nextQuestionIdx = currentQuestionIdx + 1

                if (nextQuestionIdx >= sessionQuestions.length) { // Last question
                  val summary = summarizeQuizResults(updatedAnswers)
                  val missedExplanations = updatedMissed.map { case (qt, _, ca, note, qe) =>
                    val expl = if (note.nonEmpty) note else "No specific note provided."
                    s"\n- $expl"
                  }.mkString("\n")
                  ...
                  val (updatedLog, nextSeq) = logInteraction(rawInput, response, currentLog, currentSeqNum)
                  (response, (updatedLog, nextSeq, None, currentSessionPrefs, Some("quiz_ended" -> topic), currentUserNameOpt))
                } else { // More questions
                  ...
                  val (updatedLog, nextSeq) = logInteraction(rawInput, response, currentLog, currentSeqNum)
                  (response, (updatedLog, nextSeq, Some(newQuizSession), currentSessionPrefs, Some("in_quiz" -> topic), currentUserNameOpt))
                }

              case "INCORRECT_VALID_CHOICE" =>
                val explanationPart = if (qExplanation.nonEmpty) s"\nExplanation: $qExplanation" else ""
                val feedback = s"Not quite. The correct answer was: $qCorrectAnswer.\nExplanation: $qExplanation"
                val updatedAnswers = answersSoFar :+ false
                val updatedMissed = missedSoFar :+ currentQuestionTuple
                val nextQuestionIdx = currentQuestionIdx + 1

                if (nextQuestionIdx >= sessionQuestions.length) { // Last question
                  val summary = summarizeQuizResults(updatedAnswers)
                  val missedExplanations = updatedMissed.map { case (qt, _, ca, note, qe) => // Corrected unpacking
                    val expl = if (note.nonEmpty) note else "No specific explanation or note provided."
                    s"- $expl"
                  }.mkString("\n")
                  val missedHeader = if (updatedMissed.nonEmpty) "\n\nHere's a review of the questions you missed:" else "\nGreat job!"
                  val response = s"$feedback\n$summary$missedHeader$missedExplanations\n\n${suggestionPrompts(Random.nextInt(suggestionPrompts.length))}"
              ...
              case "INVALID_CHOICE_LETTER" =>
                val validLetters = "abcd".take(qChoices.length).map(c => s"'$c'").mkString(", ")
                val response = s"Hmm, that's not a valid choice letter. Please pick from ${validLetters}.\n" + presentQuizQuestion(currentQuestionTuple, displayQuestionNum)
                val (updatedLog, nextSeq) = logInteraction(rawInput, response, currentLog, currentSeqNum)
                (response, (updatedLog, nextSeq, currentQuizSessOpt, currentSessionPrefs, lastActionCtx, currentUserNameOpt))

              case "CHOICE_NOT_FOUND" =>
                val response = "I couldn't match your answer to one of the choices. Please try selecting a letter (a, b, c, d) or ensure your typed answer is close to one of the options.\n" + presentQuizQuestion(currentQuestionTuple, displayQuestionNum)
                val (updatedLog, nextSeq) = logInteraction(rawInput, response, currentLog, currentSeqNum)
                (response, (updatedLog, nextSeq, currentQuizSessOpt, currentSessionPrefs, lastActionCtx, currentUserNameOpt))
            }
          }
        case None =>
          ...
          val (l, s) = logInteraction(rawInput, r, currentLog, currentSeqNum)
          (r, (l, s, None, currentSessionPrefs, Some("quiz_error" -> topic), currentUserNameOpt))
      }

    case None => // No quiz in progress
      val followUpKeywords = List("further","more", "details", "clarify", "example", "use case", "show me more", "tell me more", "another way", "different angle","dont understand","dive" ,"deeper")
          ...
          case Some(("explained", conceptName)) =>
            explanations.get(conceptName) match {
              case Some((expl1, expl2, exampleText, note)) =>
                ...
                return (fullResponse, (newLog, newSeqNum, None, currentSessionPrefs, Some("explained" -> conceptName), currentUserNameOpt))
              case None =>
                val r = "I seem to have forgotten what we were discussing. What topic are you interested in?"
                val (l,s) = logInteraction(rawInput, r, currentLog, currentSeqNum)
                return (r, (l,s, None, currentSessionPrefs, None, currentUserNameOpt))
            }
          case _ =>
        }
      }
        ...
        case "set_name" =>
          identifiedNameOpt match {
            case Some(name) =>
              ...
              (nameSetResponse, (l, s, None, updatedPrefs, None, Some(name.capitalize)))
            case None =>
              val r = "I think you tried to tell me your name, but I missed it. Try 'my name is [name]'."
              val (l, s) = logInteraction(rawInput, r, currentLog, currentSeqNum)
              (r, (l, s, None, currentSessionPrefs, lastActionCtx, currentUserNameOpt))
          }
        case "greeting" =>
          val nameToGreet = currentSessionPrefs.get("session_username")
          val r = greetUser(nameToGreet)
          val (l, s) = logInteraction(rawInput, r, currentLog, currentSeqNum)
          (r, (l, s, None, currentSessionPrefs, None, currentUserNameOpt))
        case "farewell" =>
          val nameToGreet = currentSessionPrefs.get("session_username").getOrElse("learner")
          val r = s"Goodbye $nameToGreet! Happy learning!" // Personalized farewell
          val (l, s) = logInteraction(rawInput, r, currentLog, currentSeqNum)
          (r, (l, s, None, currentSessionPrefs, None, currentUserNameOpt))
        case "quiz" =>
          val topicOpt = langOrConceptOpt.orElse(getUserPreferences("last_explained_concept", currentSessionPrefs))
          topicOpt match {
            case Some(topic) =>
              val (quizStartMsg, stateAfterQuizStart) = startNewQuiz(topic)
              val (_, _, _, prefsFromQuizStart, _, _) = stateAfterQuizStart // Adjusted unpacking
              val finalPrefs = storeUserPreferences("last_quiz_topic", topic, prefsFromQuizStart)
              (quizStartMsg, stateAfterQuizStart.copy(_4 = finalPrefs))
            case None =>
        ...
        case "explain" =>
          langOrConceptOpt.orElse(concepts.headOption) match {
            case Some(conceptToExplain) =>
              ...
              val (l, s) = logInteraction(rawInput, r, currentLog, currentSeqNum)
              (r, (l, s, None, updatedPrefs, Some("explained" -> canonicalConcept), currentUserNameOpt))
            case None =>
