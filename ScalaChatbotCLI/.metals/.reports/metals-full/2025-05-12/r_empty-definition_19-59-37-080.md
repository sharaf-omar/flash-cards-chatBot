error id: scala/collection/MapOps#getOrElse().
...
non-local guesses:
	 -java/io/globalQuizBank/getOrElse.
	 -java/io/globalQuizBank/getOrElse#
	 -java/io/globalQuizBank/getOrElse().
	 -scala/io/StdIn.globalQuizBank.getOrElse.
	 -scala/io/StdIn.globalQuizBank.getOrElse#
	 -scala/io/StdIn.globalQuizBank.getOrElse().
	 -globalQuizBank/getOrElse.
	 -globalQuizBank/getOrElse#
	 -globalQuizBank/getOrElse().
	 -scala/Predef.globalQuizBank.getOrElse.
	 -scala/Predef.globalQuizBank.getOrElse#
	 -scala/Predef.globalQuizBank.getOrElse().
offset: 25717
...
type QuizQuestion = (String, List[String], String, String,String) // questionText, choices, correctAnswerText, generalNote
...
val keywordVariations: Map[String, List[String]] = {
  conceptKeywords.map { concept =>
    ...
    if (concept.contains(" ")) variations = canonical.replace(" ", "") :: canonical.replace("-", " ") :: variations
...
private def loadQuizBankFromFile(filePath: String = questionsPath): QuizBank = {
    ...
    Try {
        val source = Source.fromFile(filePath, "UTF-8") // Specify UTF-8 for robustness
        ...
        Try {
            ...
            // println(s"--- DEBUG: Columns found: ${columns.length} --- For line: [$line]")
            // println(s"--- DEBUG: Column contents: ${columns.mkString(" | ")} ---")

            if (columns.length == 9) { // Concept,Question,O1,O2,O3,O4,CorrectAns,Note,Explanation
            ...
            val generalNote = columns(7).trim.stripPrefix("\"").stripSuffix("\"")         // This is the 'Note' column
            val questionExplanation = columns(8).trim.stripPrefix("\"").stripSuffix("\"") // This is the 'Explanation' column
        ...
        println(s"Failed to load quiz bank from $filePath. Using an empty quiz bank. (This message appears if the Try block failed)")
...
def handleUserInput(
  ...
  currentQuizSessOpt match {
    case Some((topic, remainingQuestions, answersSoFar, currentQuestionNum)) =>{
        val allQuestionsForTopic = globalQuizBank.getOr@@Else(topic, List.empty)
        val currentQuestionOpt = if (currentQuestionNum -1 < allQuestionsForTopic.length) {
            Some(allQuestionsForTopic(currentQuestionNum - 1)) // currentQuestionNum is 1-based
        } else None
    ...
    currentQuestionOpt match {
        case Some(currentQuestionTuple) =>
            ...
            if (input == "hint") {
                ...
                // ... (logging and returning state)
            } else if (input == "explain" || input == "explain question") { // New: User asks for question explanation
                val response = if (qExplanation.nonEmpty) s"Explanation for this question: $qExplanation\nYour answer: " else "No detailed explanation available for this specific question.\nYour answer: "
                val (updatedLog, nextSeq) = logInteraction(input, response, currentLog, currentSeqNum)
                (response, (updatedLog, nextSeq, currentQuizSessOpt, currentPrefs)) // Don't advance quiz
            }
            else {
                evaluateQuizAnswer(input, qChoices, qCorrectAnswer) match {
                    case Some(isCorrect) =>
                        ...
                        // Store if the answer was correct AND the full question for later explanation of wrong answers
                        val detailedAnswer: (Boolean, QuizQuestion) = (isCorrect, currentQuestionTuple) 
                        // answersSoFar needs to change to store detailedAnswer
                        // QuizSessionState needs to change to store List[(Boolean, QuizQuestion)]
                        // This is a significant change if we stick to the project spec's `List[Boolean]` for summarizeQuizResults.
                        // For now, let's keep answersSoFar as List[Boolean] for summarizeQuizResults
                        // and handle missed question explanations separately if desired at the end.
    
                        val updatedAnswers = answersSoFar :+ isCorrect // Stays List[Boolean] for now
    
                        // NEW: Store missed questions separately for end-of-quiz explanation
                        val updatedMissedQuestions = if (!isCorrect) {
                            chatbotState._3.flatMap(qs => chatbotState._4.get("missed_questions").map(_.asInstanceOf[List[QuizQuestion]])).getOrElse(List.empty) :+ currentQuestionTuple
                        } else {
                            chatbotState._3.flatMap(qs => chatbotState._4.get("missed_questions").map(_.asInstanceOf[List[QuizQuestion]])).getOrElse(List.empty)
                        }
                        val newPrefsWithMissed = storeUserPreferences("missed_questions", updatedMissedQuestions.asInstanceOf[AnyRef].toString, currentPrefs) // Crude storage for now
    
                        if (remainingQuestions.isEmpty) { // Last question
                            val summary = summarizeQuizResults(updatedAnswers)
                            // Explain missed questions here
                            val missedExplanations = updatedMissedQuestions.map { case (qt, _, ca, _, qe) =>
                                s"\nFor the question: \"$qt\"\nThe correct answer was: $ca\nExplanation: ${if (qe.nonEmpty) qe else "No specific explanation provided."}"
                            }.mkString("\n")
    
                            val missedHeader = if (updatedMissedQuestions.nonEmpty) "\n\nHere are explanations for questions you missed:" else ""
    
                            val response = s"$feedback\n$summary$missedHeader$missedExplanations\n\n${suggestionPrompts(Random.nextInt(suggestionPrompts.length))}"
                            val (updatedLog, nextSeq) = logInteraction(input, response, currentLog, currentSeqNum)
                            // Clear missed questions from preferences
                            val finalPrefs = storeUserPreferences("missed_questions", List.empty[QuizQuestion].asInstanceOf[AnyRef].toString, newPrefsWithMissed)
                            (response, (updatedLog, nextSeq, None, finalPrefs)) // End quiz
                        } else {
                            // ... (rest of the logic to present next question)
                            ...
                            val (updatedLog, nextSeq) = logInteraction(input, response, currentLog, currentSeqNum)
                            // Pass along newPrefsWithMissed to the next state
                            (response, (updatedLog, nextSeq, Some(newQuizSession), newPrefsWithMissed))
                        }
                    // ...
                }
            }
        case None => // Should not happen if quiz state is valid
...
def runChatbot(
  ...
  def loop(currentState: ChatbotState): Unit = {
    print("\n> ")
    Try(readLine()) match {
        // case Success(null) | Success("exit") | Success("quit") | Success("bye") => // OLD
        case scala.util.Success(userInputString) => // NEW - capture the string first
          if (userInputString == null || farewellKeywords.contains(userInputString.toLowerCase)) { // NEW - check conditions
            println("Goodbye! Happy learning!")
            persistentLogInteraction(if (userInputString == null) "EOF" else userInputString, "Chatbot session ended.")
          } else {
            val (response, nextState) = handleUserInput(userInputString, currentState, initialExplanations, initialQuizBank)
            println(response)
            if (!farewellKeywords.contains(userInputString.toLowerCase)) {
              loop(nextState)
            } else {
              persistentLogInteraction(userInputString, response)
            }
          }
        // case Failure(ex) => // OLD
        case scala.util.Failure(ex) => // NEW
