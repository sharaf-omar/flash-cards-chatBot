...
empty definition using pc, found symbol in pc: scala/collection/immutable/List#filter().
...
//(IntentString, List[Concept, Language, UserName)
...
def determineIntent(tokens: List[String]): IntentInfo = {
  ...
  val foundLanguagesOrTopics = lowerTokens.fil@@ter(t => languageKeywords.contains(t) || conceptKeywords.contains(t)).map(t => variationToCanonical.getOrElse(t, t)) 
