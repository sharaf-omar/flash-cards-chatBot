  ...
  Try {
    ...
    lines.flatMap { line =>
      Try {
        ...
        val columns = line.split(",(?=(?:[^\"]*\"[^\"]*\")*[^\"]*$)", -1)
        println(s"--- DEBUG CSV LINE PARSED ---")
        println(s"Raw Line: [$line]")
        println(s"Columns (${columns.length}): ${columns.mkString(" ## ")}")
        if (columns.length == 9) {
          ...
          println(s"Concept: $csvConcept, Question: $questionText, Parsed Correct Answer: $correctAnswer")
