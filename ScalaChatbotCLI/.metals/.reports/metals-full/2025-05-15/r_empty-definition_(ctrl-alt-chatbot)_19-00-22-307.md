error id: `<none>`.
file:///C:/Users/dr_sh/OneDrive/Desktop/CTRL%20ALT%20CHATBOT/CTRL%20ALT%20CHATBOT/src/main/scala/chatbot.scala
...
offset: 7533
uri: file:///C:/Users/dr_sh/OneDrive/Desktop/CTRL%20ALT%20CHATBOT/CTRL%20ALT%20CHATBOT/src/main/scala/chatbot.scala
text:
```scala
import java.io._
...
val conceptKeywords = List(
    ...
    "time complexity", "space complexity","ai","cyber security","web programming", "software engineer","data scientist","machine learning engineer","devops engineer","clo@@ud engineer", "cybersecurity analyst","game developer", "mobile developer","full stack developer"
...
type ChatbotState = (InteractionLog, Int, Option[QuizSessionState], UserPreferences,LastActionContext) 
...
def handleUserInput(
  ...
  val (currentLog, currentSeqNum, currentQuizSessOpt, currentSessionPrefs, lastActionCtx) = chatbotState
  val input = rawInput.trim.toLowerCase

  persistentLogInteraction(input, "N/A - User Input Processing", Some("RAW_INPUT")) // Assuming persistentLogInteraction exists
  ...
  def startNewQuiz(topic: String, numQuestions: Int = 5): (String, ChatbotState) = {
    selectQuizQuestions(topic, numQuestions, quizBank) match {
      case Some(selectedQuestions) if selectedQuestions.nonEmpty =>
        ...
        val newQuizSession: QuizSessionState = (topic, selectedQuestions, List.empty[Boolean], 0, List.empty[QuizQuestion]) // currentQuestionIdx is 0-based
        ...
        (response, (updatedLog, nextSeq, Some(newQuizSession), currentSessionPrefs, Some("quiz_started" -> topic)))
      case _ =>
        ...
        val (updatedLog, nextSeq) = logInteraction(rawInput, response, currentLog, currentSeqNum)
        (response, (updatedLog, nextSeq, None, currentSessionPrefs, lastActionCtx))
    }
  }
  ...
  currentQuizSessOpt match {
    case Some((topic: String, sessionQuestions: List[QuizQuestion], answersSoFar: List[Boolean], currentQuestionIdx: Int, missedSoFar: List[QuizQuestion])) =>
      val displayQuestionNum = currentQuestionIdx + 1 // For user display (1-based)

      val currentQuestionOpt: Option[QuizQuestion] =
        if (currentQuestionIdx >= 0 && currentQuestionIdx < sessionQuestions.length) {
          Try(sessionQuestions(currentQuestionIdx)).toOption
        } else {
          None
        }
      ...
      currentQuestionOpt match {
        case Some(currentQuestionTuple: QuizQuestion) =>
          ...
          // println(s"--- DEBUG HANDLE_USER_INPUT (Quiz Q $displayQuestionNum) ---")
          // println(s"Current Question Being Processed: ${qText}")
          // println(s"CORRECT ANSWER for this Question (qCorrectAnswer): [${qCorrectAnswer}]")
          // println(s"--------------------------------")
          ...
          if (input == "hint") {
            ...
            val (updatedLog, nextSeq) = logInteraction(rawInput, response, currentLog, currentSeqNum)
            (response, (updatedLog, nextSeq, currentQuizSessOpt, currentSessionPrefs, lastActionCtx))
            ...
            // println(s"--- DEBUG EVALUATE_CALL (Quiz Q $displayQuestionNum) ---")
            // println(s"User's Raw Input for Evaluation: [${input}]")
            // println(s"Choices Passed to Evaluate: ${qChoices.mkString(", ")}")
            // println(s"Correct Answer Text PASSED to Evaluate: [${qCorrectAnswer}]")
            // println(s"-----------------------------")
            evaluateQuizAnswer(input, qChoices, qCorrectAnswer) match {
              case Some(isCorrect: Boolean) =>
                ...
                if (nextQuestionIdx >= sessionQuestions.length) { // Last question was just answered
                  ...
                  val (updatedLog, nextSeq) = logInteraction(rawInput, response, currentLog, currentSeqNum)
                  (response, (updatedLog, nextSeq, None, currentSessionPrefs, Some("quiz_ended" -> topic)))
                  ...
                  val (updatedLog, nextSeq) = logInteraction(rawInput, response, currentLog, currentSeqNum)
                  (response, (updatedLog, nextSeq, Some(newQuizSession), currentSessionPrefs, Some("in_quiz" -> topic)))
                }
        ...
        case None => // Should not happen if currentQuestionIdx is valid
          ...
          (r, (l, s, None, currentSessionPrefs, Some("quiz_error" -> topic)))
      }

    case None => // No quiz in progress
      val followUpKeywords = List("more", "details", "clarify", "example", "use case", "show me more", "tell me more", "another way", "different angle","dont understand")
          ...
          case Some(("explained", conceptName)) =>
            explanations.get(conceptName) match {
              case Some((expl1, expl2, exampleText, note)) =>
                ...
                return (fullResponse, (newLog, newSeqNum, None, currentSessionPrefs, Some("explained" -> conceptName)))
              case None =>
                val r = "I seem to have forgotten what we were discussing. What topic are you interested in?"
                val (l,s) = logInteraction(rawInput, r, currentLog, currentSeqNum)
                return (r, (l,s, None, currentSessionPrefs, None))
            }
          case _ => // No relevant context for this type of follow-up, fall through
        }
      }
      ...
      val (responseString, newChatbotStateTuple) = intent match {
        case "greeting" =>
          val r = greetUser()
          val (l, s) = logInteraction(rawInput, r, currentLog, currentSeqNum)
          (r, (l, s, None, currentSessionPrefs, None))
        case "farewell" =>
        ...
        case "quiz" =>
          langOrConceptOpt match {
            ...
            case None =>
              ...
              val (l, s) = logInteraction(rawInput, r, currentLog, currentSeqNum)
              (r, (l, s, None, currentSessionPrefs, lastActionCtx))
          }
        case "explain" =>
          langOrConceptOpt.orElse(concepts.headOption) match {
            case Some(conceptToExplain) =>
              ...
              val (l, s) = logInteraction(rawInput, r, currentLog, currentSeqNum)
              (r, (l, s, None, currentSessionPrefs, Some("explained" -> canonicalConcept)))
            case None =>
...
def runChatbot(
  ...
  val initialBotState: ChatbotState = (initialLog, initialSeqNum, None, Map.empty, None)
