  ...
  val singleWordActionTokens = textAfterConceptExtraction
    ...
    .flatMap { token =>
      ...
      variationToCanonical.get(token) match {
        case Some(canonical) if conceptKeywords.contains(canonical) => Some(canonical) // It's an alias for a known concept
        case Some(otherCanonical) if knownKeywords.contains(otherCanonical) => Some(otherCanonical) // Alias for other keyword
        case None => if (knownKeywords.contains(token)) Some(token) else None // It's a direct known keyword
      }
    }
    .toSet
